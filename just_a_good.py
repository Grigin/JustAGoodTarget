...
def similar_words(key_words, model, lim = 300):
    if len(key_words) >= lim:
        ...
        similar_words = sorted(Counter([word[0] for word in similar_words]).items(), key=operator.itemgetter(1))[:len(similar_words) // 2]
        similar_words = sorted([(pair[0], dist_by_vector(key_words, pair[0], model)) for pair in similar_words], key=operator.itemgetter(1))[len(similar_words) // 2:]
...
stop_words = stopwords.words('russian')
mapping = {'NOUN': "NOUN", "ADJF": "ADJ", "ADJS": "ADJ", "COMP": "ADJ", "VERB": "VERB", "INFN" : 'VERB', "PRTF": "VERB", "PRTS": "VERB", "GRND": "VERB", "NUMR": "NUM", "ADVB": "ADV", "NPRO": "NOUN", "PRED": "ADV", "PREP": "PRON", "CONJ": "SCONJ", "PRCL": "PART", "INTJ": "INTJ"}
...
class TARGET():
    def __init__(self, data, key_words, morph, model, mapping):
        ...
        self.group_cloud = Group_cloud(discription, model)
        self.data = data
    ...
    def make_target(self):
        ...
        return [i[0] for i in scores[::-1]]

with open("final.txt"):
    data = [json.loads(line.strip()) for line in f]


mod = TARGET(data, key_words, morph, mapping)
json.dumps(make_target()[:100])
with open('recipients.txt', 'w') as f:
    f.write(json.dumps(make_target()[:100]))
